...
class Browser_profile:
    ...
    def scrape_and_save_driver(self, country, user_agent, proxy_zone, country_id, crawler_method_, proxy_service, browser_name):
        try:
            self.__logger.info(f" --- scrape and save driver settings ---")
            driver_used = f"Playwright-{browser_name}"
            profile = f'default_{country}'
            crawler_method = f"{crawler_method_} {constants.crawler_version}"
