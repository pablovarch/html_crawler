...
class Domain_features:
    ...
    def manage_feature(self, domain_id, date_, input_features, domain_item):         
        ...
        if self.check_invalid_html(input_features['html_text']):
            ...
            if features == []:
                ...
                self.insert_feature(domain_id, date_, input_features, domain_item)            
            elif features[0]['domain_id'] == domain_id and features[0]['last_update'] != date_:
                # update
                self.__logger.info
                (f"--- update features domain - {domain_id} ---")
                self.update_features(domain_id, date_, input_features, domain_item)
            else:
    ...
    def insert_feature(self, domain_id, date_, input_features, domain_item):
        # Try to connect to the DB
        try:
            conn = psycopg2.connect(host=db_connect['host'],
                                    database=db_connect['database'],
            ...
            sql_string = """INSERT INTO public.domain_features (domain_id, homepage_button, num_popups, html_text, last_update ,
                            terms_of_use, dmca, privacy_policy, domain_name)
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s);"""

            try:
                # Try to execute the sql_string to save the data
                cursor.execute(sql_string, (domain_id, input_features['homepage_button'], input_features['num_popups'],
                                            input_features['html_text'], date_ , input_features['terms_of_use'],
                                            input_features['dmca'], input_features['privacy_policy'],domain_item))
                conn.commit()
            except Exception as e:
    ...
    def update_features(self, domain_id, date_, input_features, domain_item):
        # Try to connect to the DB
        try:
            conn = psycopg2.connect(host=db_connect['host'],
                                    database=db_connect['database'],
            ...
            sql_string = """UPDATE public.domain_features SET homepage_button=%s, num_popups=%s, html_text=%s,
                            last_update=%s, terms_of_use=%s, dmca=%s, privacy_policy=%s, domain_name=%s
                            WHERE domain_id=%s;"""
